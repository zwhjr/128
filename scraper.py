...
for table in BeautifulSoup.find_all("table", attrs={"class": "wikitable"}):
    temp_list=[]
    tr_tags = table.find_all("tr")
    
    for td_tag in tr_tags.find_all("td", attrs={"class": "wikitable"}):

        for td_tag in tr_tags:
            temp_list.append(td_tag)            
            
brown_dwarfs.append(temp_list)
brown_dwarfs.to_csv("brown_dwarfs.csv")
